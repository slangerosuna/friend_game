                ...
                for mut entity in query.iter_mut() {
                    if entity.1.static_id == static_id {
                        ...
                        while i < message.data.len() {
                            ...
                            let len = u16::from_le_bytes([message.data[i], message.data[i + 1]]) as usize;
                            i += 2;
    ...
    for entity in query.iter() {
        if (entity.1.object_info & 0b01000000) != 0 {
            ...
            for component in entity.0 {
                ...
                // Max component size is 65535 bytes
...
impl NetworkingState {
    ...
    pub fn get_event_in<T: for<'a> Deserialize<'a>> (
        &self,
        event_code: usize,
    ) -> impl Iterator<Item = T> + use<'_, T> {
        let mut guard = self.event_queue_in[event_code].lock().unwrap();
        let drain: Vec<NetworkingEvent> = guard.drain(..).collect();
        drop(guard);

        drain.into_iter().map(|event| {
            bincode::deserialize(&event.data).unwrap()
        })
    }

    pub fn queue_event_out<T: Serialize>(
        &self,
        event: T,
        event_code: usize,
    ) {
        let mut serialized = bincode::serialize(&event).unwrap();
        serialized.insert(0,event_code as u8);
        
        let event = NetworkingEvent::new(Event, serialized);
